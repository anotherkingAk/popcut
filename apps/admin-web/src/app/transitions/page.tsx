...
interface Transition {
  id: string; name: string; type: string; duration: number; category: string
...
export default function TransitionsPage() {
  ...
  const [form, setForm] = useState({ name: '', type: 'dissolve', duration: 500, category: 'general', isPublic: true, isPremium: false })

  const load = async () => {
    try {
      const data = await apiGet<{ data: Transition[] }>('/admin/transitions?page=1&limit=50')
      setItems(data.data)
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault()
    await apiMutate('POST', '/admin/transitions', { ...form, config: {} })
    setShowForm(false)
    setForm({ name: '', type: 'dissolve', duration: 500, category: 'general', isPublic: true, isPremium: false })
    load()
  }

  const togglePublish = async (item: Transition) => {
    const endpoint = item.isPublic ? 'unpublish' : 'publish'
    await apiMutate('PUT', `/admin/transitions/${item.id}/${endpoint}`)
    load()
  }

  const handleDelete = async (id: string) => {
    if (!confirm('Delete this transition?')) return
    await apiMutate('DELETE', `/admin/transitions/${id}`)
  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-text">Transitions</h1><p className="text-sm text-text-muted">Manage video transitions</p></div>
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-primary text-white text-sm hover:bg-primary-hover"><Plus className="w-4 h-4" /> Add Transition</button>
      </div>
      {showForm && (
        <form onSubmit={handleCreate} className="mb-6 p-4 rounded-xl bg-surface border border-border space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <input placeholder="Type (dissolve/wipe/slide/fade)" value={form.type} onChange={e => setForm({...form, type: e.target.value})} className="h-9 rounded-lg bg-background border border-border px-3 text-sm text-text outline-none focus:border-primary" />
            <input type="number" placeholder="Duration (ms)" value={form.duration} onChange={e => setForm({...form, duration: parseInt(e.target.value) || 500})} className="h-9 rounded-lg bg-background border border-border px-3 text-sm text-text outline-none focus:border-primary" />
          </div>
      ...
      <div className="rounded-xl bg-surface border border-border overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border text-text-muted text-xs uppercase">
            ...
            <th className="text-left px-4 py-3 font-medium">Duration</th><th className="text-left px-4 py-3 font-medium">Category</th>
          ...
          <tbody>
            {items.map(item => (
              <tr key={item.id} className="border-b border-border last:border-0 hover:bg-surface-hover">
                ...
                <td className="px-4 py-3 text-text-muted">{item.duration}ms</td>
                ...
                <td className="px-4 py-3 text-right flex items-center justify-end gap-1">
                  <button onClick={() => togglePublish(item)} className="p-1.5 rounded hover:bg-surface-hover text-text-muted hover:text-success" title="Toggle publish">{item.isPublic ? <XCircle className="w-4 h-4" /> : <CheckCircle className="w-4 h-4" />}</button>
