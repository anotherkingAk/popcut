...
export default function ExportJobsPage() {
  ...
  useEffect(() => {
    ...
    api.getExportJobs().then(r => setJobs(r.data)).catch(console.error).finally(() => setLoading(false))
  }, [])
  ...
  return (
    <AdminLayout>
      <div className="mb-6"><h1 className="text-2xl font-bold text-text">Export Jobs</h1><p className="text-sm text-text-muted">Track video export progress</p></div>
      <div className="rounded-xl bg-surface border border-border overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border text-text-muted text-xs uppercase">
            <th className="text-left px-4 py-3 font-medium">User</th><th className="text-left px-4 py-3 font-medium">Project</th>
            <th className="text-left px-4 py-3 font-medium">Format</th><th className="text-left px-4 py-3 font-medium">Status</th>
            <th className="text-left px-4 py-3 font-medium">Progress</th><th className="text-left px-4 py-3 font-medium">Created</th>
          </tr></thead>
          <tbody>
            {jobs.map(job => (
              <tr key={job.id} className="border-b border-border last:border-0 hover:bg-surface-hover">
                ...
                <td className="px-4 py-3 text-text-muted">{job.project?.name || 'Unknown'}</td>
                <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full bg-surface-hover text-text-secondary text-xs uppercase">{job.format}</span></td>
